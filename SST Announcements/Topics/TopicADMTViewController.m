...
#import "TopicADMTViewController.h"
...
@implementation TopicADMTViewController
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
        ...
        NSURL *url = [NSURL URLWithString:@"http://sst-students2013.blogspot.sg/feeds/posts/default/-/ADMT?alt=rss"];
...
{
    [self performSegueWithIdentifier:@"ADMTToDetail" sender:self]; //Perform the segue
    [tableView deselectRowAtIndexPath:indexPath animated:YES]; //Deselect the row automatically
}

- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([[segue identifier] isEqualToString:@"ADMTToDetail"])
    {        
